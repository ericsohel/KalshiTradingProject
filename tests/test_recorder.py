...
class Harness:
    ...
    def session(self, url: str, *, conn_id: int) -> WsSession:
        ...
        # trip the silence timeout; the query string tells the fake which connection it is.
        return WsSession(
            f"{url}?conn={conn_id}",
            StubSigner(),
...
async def test_a_failed_universe_refresh_keeps_the_plan_and_is_retried(
    tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    async with FakeKalshiWs() as fake, recording(fake.url, tmp_path) as harness:
        harness.rest.answers["/markets"].insert(0, httpx.Response(503))
        harness.start()
        books = await harness.connection(fake, 2)
