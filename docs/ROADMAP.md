...
| M4 | Bake, prune, and catalog | `tape bake`, `tape prune` (ADR 0025), manifests, `Catalog.book_at` | Bake is idempotent (sha256 stable) and accounts for every raw record; prune deletes only hours meeting ADR 0025; `book_at` equals the recorder's live book at keyframe instants; first manifest published |
